...
func TestNewSettings(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			envVars: map[string]string{
				...
				"PLUGIN_OUTPUT_FILE":         "./custom-output/prompt.txt",
				"PLUGIN_REVIEW_OUTPUT_FILE":  "./custom-output/ai-review.json",
			...
			expected: Settings{
				...
				EnableCodeSmell:  true,
				CommentCount:     25,
				OutputFile:       "./custom-output/prompt.txt",
				ReviewOutputFile: "./custom-output/ai-review.json",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if settings.OutputFile != tt.expected.OutputFile {
				t.Errorf("OutputFile = %v, want %v", settings.OutputFile, tt.expected.OutputFile)
			}
			if settings.ReviewOutputFile != tt.expected.ReviewOutputFile {
				t.Errorf("ReviewOutputFile = %v, want %v", settings.ReviewOutputFile, tt.expected.ReviewOutputFile)
			}
